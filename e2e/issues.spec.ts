...
test.describe("Issues Page", () => {
  test("should display issues page correctly", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: /Find Good First Issues/i }),
    ).toBeVisible();
  });
  ...
  test("should filter issues by language", async ({ page }) => {
    ...
    test
      ...
      .annotations.push({
        type: "skip-ci",
        description: "Requires API connection",
      });
    ...
    try {
      ...
      await page
        .waitForSelector('text="0 results found"', { timeout: 5000 })
        .catch(() => {});
      console.log(
        "No issue cards found, but test continues as this may be expected in test environment",
      );
    }
  });
  ...
  test("should search issues by keyword", async ({ page }) => {
    ...
    await page
      .locator('[data-testid="keyword-search-input"]')
      .fill("documentation");
...
test.describe("Saved Issues", () => {
  test("should require authentication to access saved issues or show auth UI", async ({
    page,
  }) => {
    ...
    try {
      ...
      if (githubButton || signInText) {
        ...
        const loginPrompt =
          (await page.getByText(/sign|log|auth|please/i).count()) > 0;
